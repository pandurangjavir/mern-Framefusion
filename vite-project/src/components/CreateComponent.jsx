...
const CreateComponent = ({ info, current_component, removeComponent }) => {
      ...
      >
        ...
        {current_component.id === info.id && (
          <div
            ...
            className="px-3 bg-white absolute top-0 hidden group-hover:block cursor-pointer py-2 rounded-md"
          >
            <BsTrash />
          </div>
        )}
  ...
  if (info.name === 'shape' && info.type === 'circle') {
    html = (
        <div id={randValue} onClick={() => info.setCurrentComponent(info)} style={{
            left: info.left + 'px',
            top: info.top + 'px',
            zIndex: info.z_index,
            transform: info.rotate ? `rotate(${info.rotate}deg)` : 'rotate(0deg)'
